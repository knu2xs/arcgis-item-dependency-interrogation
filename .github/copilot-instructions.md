...
- **SQL/DuckDB**: Externalise non-trivial queries to `src/arcgis_dependency/sql/*.sql`. Always use bind parameters — never f-string interpolation.
...
- The primary Python package is `arcgis_dependency`, located in `src/arcgis_dependency/`
- ArcGIS Pro toolboxes are in `arcgis/` and reference `arcgis_dependency` for shared logic
...
- **`src/arcgis_dependency/`** - Main source code for the project package
    - **`src/arcgis_dependency/sql/`** - Externalised SQL queries (`*.sql`) used
...
`src/arcgis_dependency/config.py`, which exposes a singleton `ConfigNode` supporting
...
Import the pre-built singletons from `arcgis_dependency.config`:
...
`src/arcgis_dependency/config.py` for a persistent local override.
...
`pyproject.toml` declares the packages that **`arcgis_dependency` itself requires to
run**. Add a dependency here when:

- A module inside `src/arcgis_dependency/` contains a top-level `import` of that package
...
runtime dependency of `arcgis_dependency`:
...
| Scenario | Add to |
|---|---|
| `import foo` inside `src/arcgis_dependency/` | `pyproject.toml` `[project] dependencies` |
...
- Use the `@with_temp_fgdb` decorator from `arcgis_dependency.utils` —
  implemented in `src/arcgis_dependency/utils/_data.py` and included by default
...
```python
from arcgis_dependency.utils import with_temp_fgdb
...
`src/arcgis_dependency/sql/`. This is a widely adopted best practice because it:
...
```python
import duckdb
from arcgis_dependency.sql import load_sql
...
- Mirror the `src/arcgis_dependency/` package structure in `testing/`:
    - One test file per module: `testing/test_<module_name>.py`
    - Example: `src/arcgis_dependency/analysis.py` → `testing/test_analysis.py`
- Use `testing/test_arcgis_dependency.py` for package-level smoke tests
...
- Run with coverage (if `pytest-cov` is installed):
  `pytest --cov=arcgis_dependency --cov-report=term-missing`
...
`src/arcgis_dependency/utils/_logging.py` and included by default in generated
...
Every module in `src/arcgis_dependency/` should configure a logger at the top of
...
- **No secrets in notebooks.** Use `secrets.*` from `arcgis_dependency.config`
...
- **Promote reusable code out of notebooks.** Once a function works, move it into
  `src/arcgis_dependency/` and import it back into the notebook. Notebooks
...
shell commands, and other important information, read specs/001-webgis-item-dependencies/plan.md
<!-- SPECKIT END -->
